...
export class DossierEtudiantComponent implements OnInit {
  ...
  updateEtudiant() {
    ...
    this.etudiantSrv.update(this.updateForm).subscribe(
      (data: any) => {
        this.etudiant = data;
        this.updateForm = data;
        this.modalRef.close(data);
        this.etudiantSrv.httpSrv.notificationSrv.showSuccess(
  ...
  onUploadEnd(documentEtudiant: DocumentEtudiant) {
    this.etudiantSrv.httpSrv.notificationSrv.showSuccess(
      "Document chargé avec succés."
    );
  }

  sendEmail() {
    this.etudiantSrv.sendEmail(this.etudiant, this.email).subscribe(
      (data: any) => {
        this.etudiant = data;
        this.closeModal1();
  ...
  verifierInscriptionEtudiantActif() {
    this.etudiantSrv
      .verifierInscriptionEtudiantActif(this.etudiant.cni)
      .subscribe(
        (data: any) => {
  ...
  loadPreinscriptionActives() {
    this.preinscriptionSrv
      .findActivePreinscriptionByEtudiant(this.etudiant)
      .subscribe(
        (data: any) => {
